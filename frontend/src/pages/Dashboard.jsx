  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard title="Overall Competency" value="76%" subtitle="+2% from last month" icon={TrendingUp} colorClass="bg-gradient-to-br from-primary-50 to-primary-100 text-primary-600 border border-primary-200" />
        ...
        <StatCard title="Skill Badges" value="8" subtitle="Top 15% in department" icon={Award} colorClass="bg-gradient-to-br from-secondary-50 to-secondary-100 text-secondary-600 border border-secondary-200" />
        <StatCard title="Learning Hours" value="45h" subtitle="In the last 30 days" icon={Clock} colorClass="bg-gradient-to-br from-orange-50 to-orange-100 text-orange-600 border border-orange-200" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-8">
          ...
          <div className="h-80 relative z-10">
            <ResponsiveContainer width="100%" height="100%">
              <RadarChart cx="50%" cy="50%" outerRadius="80%" data={competencyData}>
                ...
                <Radar name="Competency" dataKey="A" stroke="#3b82f6" strokeWidth={3} fill="url(#colorUv)" fillOpacity={0.4} />
                <defs>
        ...
        <div className="bg-white/80 backdrop-blur-md p-8 rounded-3xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border border-white/50 relative overflow-hidden hover:border-primary-200 transition-colors">
          ...
          <div className="space-y-4 relative z-10">
            {[
              { title: 'Advanced Survey Design', provider: 'iGOT Karmayogi', match: '98%', gap: 'Critical' },
              { title: 'Python for Data Science', provider: 'NSSTA TPAC', match: '85%', gap: 'High' },
              { title: 'Leadership in Tech', provider: 'GovLearn', match: '72%', gap: 'Medium' }
            ].map((course, idx) => (
              <div key={idx} className="flex items-center justify-between p-5 rounded-2xl bg-white border border-gray-100 hover:border-primary-300 hover:shadow-md transition-all cursor-pointer group">
                <div>
                  <h3 className="font-bold text-gray-900 group-hover:text-primary-600 transition-colors text-lg">{course.title}</h3>
                  <p className="text-sm font-medium text-gray-500 mt-1">{course.provider}</p>
                </div>
                <div className="text-right">
                  ...
                  <p className="text-sm font-black text-gray-800 mt-3">{course.match} Match</p>
                </div>
              </div>
            ))}
          </div>
